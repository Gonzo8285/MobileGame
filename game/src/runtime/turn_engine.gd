...
static func _resolve_friendly_attacks_in_lane(lane: Lane) -> int:
	...
	for u in lane.friendly_units:
		...
		# On-hit keyword application — BLEED/POISON for B2.7, LIFESTEAL from
		# AURA.E1+LIFESTEAL.E1. Checks go via UnitInstance.has_keyword() (not
		# card_data) so aura-granted keywords fire too — Wolf-Tokens under
		# W42 Den-Mother's aura heal on attack via runtime LIFESTEAL grant.
		if u.card_data != null:
			...
			# LIFESTEAL — heal attacker for damage_dealt, clamped at max_hp()
			# via UnitInstance.heal(). dealt > 0 gate so 0-damage swings don't
			# spuriously heal. See keywords/lifesteal_v0.md.
