# Per-card art spec — The Vow-Broken Magus
...
- **card_id:** W6
- **display_name:** The Vow-Broken Magus
...
```
twice-excommunicated penitent who took the Court's necromantic vow to
atone faster, brass execution-mask split open down the middle revealing
court-shroud beneath, one gloved hand one skeletal
```
...
> **Deviation note:** Hybrid Iron Penitents × Ash-Mourners — primary faction = Iron Penitents,
court-shroud + raven-quill secondary motifs layered through subject text rather
than additional LoRA. If first pass under-reads as hybrid, escalate to dual-LoRA
stack as a flagged proposal in pipeline_spec_proposals.md.
...
- **seed:** 64242
- **rationale:** Warlord #6, formula `warlord_number × 10000 + 4242` (per `_template.md` Warlord rule). Sits clear of all five faction common-card seed ranges.
...
- **output_path:** `art/warlords/w6_the_vow_broken_magus.png`
- **iterations_path:** `art_iterations/W6_the_vow_broken_magus/`
...
| Date | Iteration | Note |
|---|---|---|
| 2026-05-XX | seed64242_v1 | first pass — accept / iterate |
